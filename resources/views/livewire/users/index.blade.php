<section class="app-user-list">
    ...
    <div class="card">
        ...
        <div class="d-flex justify-content-between align-items-center mx-50 row pt-0 pb-2">
            ...
            @php
                $adminStatus = App\Models\User::STATUS;
                $searchableArray = [];
                    ...
                    <div class="col-sm-12 col-md-8 col-lg-6 ps-xl-75 ps-0">
                        <div
                            ...
                            <div class="dt-buttons btn-group flex-wrap"><a class="btn add-new btn-primary mt-50"
                                    tabindex="0"
                                    href="{{ route('users.create') }}"><span>{{ __('Add New User') }}</span></a>
                            </div>
                        </div>
                    </div>
                </div>
                <table class="user-list-table table dataTable no-footer dtr-column" id="DataTables_Table_0"
                    ...
                    <thead class="table-light">
                        <tr role="row">
                            ...
                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1"
                                colspan="1" style="width: 120.25px;"
                                aria-label="User: activate to sort column ascending">{{ __('User') }}</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($users as $user)
                            <tr>
                                <td>
                                    @if ($user->image)
                                        <div class="avatar avatar-lg">
                                            <img src="{{ Storage::url($user->image) }}" alt="{{ $user->fname }}">
                                        </div>
                                    @else
                                        <div
                                            class="avatar bg-light-{{ getRandomValue(App\Models\Admin::AVATAR_COLORS) }} avatar-lg">
                                            <span
                                                class="avatar-content">{{ $user->fname[0] . $user->lname[0] }}</span>
                                        </div>
                                    @endif
                                </td>
                                ...
                                @if (authorized('Delete user') || authorized('Edit user'))
                                    <td>
                                        <div class="dropdown">
                                            <button type="button"
                                            ...
                                            <div class="dropdown-menu" style="">
                                                ...
                                                @if (authorized('Delete user'))
                                                    <a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                        wire:click="saveAdminWillDeleted('{{ Crypt::encrypt($user->id) }}')"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myModalLabel120">{{ __('Delete User') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        {{ __('After you delete this user, system can keep his information for 30 days only. You can back them up before end this time.') }}
                    </div>
