<section class="app-user-list">
    ...
    <div class="card">
        ...
        <div class="d-flex justify-content-between align-items-center mx-50 row pt-0 pb-2">
            ...
            @php
                $adminStatus = App\Models\Admin::STATUS;
                $searchableArray = [];
                foreach ($adminStatus as $status) {
                    # code...
                    $searchableArray[$status] = ucfirst(__($status));
                }
            @endphp
                    ...
                    <div class="col-sm-12 col-md-8 col-lg-6 ps-xl-75 ps-0">
                        <div
                            ...
                            <div class="dt-buttons btn-group flex-wrap"><a class="btn add-new btn-primary mt-50"
                                    tabindex="0"
                                    href="{{ route('admins.create') }}"><span>{{ __('Add New Admin') }}</span></a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <thead class="table-light">
                        <tr role="row">
                            ...
                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1"
                                ...
                                aria-label="User: activate to sort column ascending">{{ __('Photo') }}</th>
                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1"
                                colspan="1" style="width: 120.25px;"
                                aria-label="User: activate to sort column ascending">{{ __('Admin') }}</th>
                            <th class="sorting sorting_desc" tabindex="0" aria-controls="DataTables_Table_0"
                                rowspan="1" colspan="1" style="width: 129.15px;" aria-sort="descending"
                                aria-label="Email: activate to sort column ascending"> {{ __('Email') }} </th>
                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1"
                                colspan="1" style="width: 120.225px;"
                                aria-label="Role: activate to sort column ascending"> {{ __('Role') }} </th>
                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1"
                                colspan="1" style="width: 118.912px;"
                                aria-label="Plan: activate to sort column ascending"> {{ __('Phone') }} </th>
                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1"
                                colspan="1" style="width: 147.775px;"
                                aria-label="Status: activate to sort column ascending"> {{ __('Gender') }} </th>
                            @if (authorized('Delete admin') || authorized('Edit admin'))
                    ...
                    <tbody>
                        ...
                        @foreach ($admins as $admin)
                            <tr>
                                <td>
                                    @if ($admin->image)
                                        <div class="avatar avatar-lg">
                                            <img src="{{ Storage::url($admin->image) }}" alt="{{ $admin->fname }}">
                                        </div>
                                    @else
                                        <div
                                            class="avatar bg-light-{{ getRandomValue(App\Models\Admin::AVATAR_COLORS) }} avatar-lg">
                                            <span
                                                class="avatar-content">{{ $admin->fname[0] . $admin->lname[0] }}</span>
                                        </div>
                                    @endif
                                </td>
                                ...
                                <td>{{ $admin->phone }}</td>
                                <td>{{ $admin->gender == 'm' ? __('Male') : __('Female') }}</td>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" style="">
                                                ...
                                                @if (authorized('Edit admin'))
                                                    <a class="dropdown-item"
                                                        href="{{ route('admins.edit', Crypt::encrypt($admin->id)) }}">
                                                ...
                                                @if (authorized('Delete admin'))
                                                    <a class="dropdown-item" href="#" data-bs-toggle="modal"
                                                        wire:click="saveAdminWillDeleted('{{ Crypt::encrypt($admin->id) }}')"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="myModalLabel120">{{ __('Delete Admin') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        {{ __('After you delete this admin, system can keep his information for 30 days only. You can back them up before end this time.') }}
                    </div>
